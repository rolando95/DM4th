...
namespace DM4th
{
    ...
    public:
        ...
        template <class U>
        explicit operator _number<U>() const {
            return _number<U>((U)this->real(), (U)this->imag());
        }

        inline explicit operator int() const {
            return (int)std::round(this->real());
        }
        ...
        template<class U>
        inline bool isEqualTo(const _number<U> &n, T tolerance = std::numeric_limits<T>::epsilon());
    };
    ...
    template <class T>
    inline _number<T> trunc(const _number<T> &value, int p = 0) {
        return truncate(value, p);
    }
