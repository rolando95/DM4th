...
namespace DM4th
{
    ...
    std::ostream& operator<<(std::ostream& stream, _number<T> n) {
        if (n.real() == INF) {
            stream<<"INF"; return stream;
        }
        else if (n.real() == -INF) {
            stream<<"-INF"; return stream;
        }
        else if (std::isnan(n.real())) {
            stream<<"NAN"; return stream;
        }

        n = round(n, DM4thConfig::outputNumberPrecision); // Es necesario corregir problemas de redondeo
        //stream
        if (n==0) {
    ...
    template<class T>
    template<class U>
    inline bool _number<T>::isEqualTo(const _number<U> &n, T tolerance) {
        if (this->real()==INF && n.real()==INF) return true;
        return std::abs(this->real() - n.real()) < tolerance && std::abs(this->imag() - n.imag()) < tolerance;
    }
    ...
    {
        return n1.isEqualTo(_number<T>(n2));
    }

    template<class U, typename T, enable_if_is_number(U, _number<T>) >
    inline bool operator==(U n1, _number<T> n2)
    {
        return _number<T>(n1).isEqualTo(n2);
    }
    ...
    template<class T>
    inline _number<T> log(_number<T> n, _number<T> base) {
        return ln(n)/ln(base);
    }
    ...
    template<class T>
    inline _number<T> sin(_number<T> n) {
        return (isReal(n))?std::sin(n.real()) : (pow(e, DM4th_i*n)-pow(e, -DM4th_i*n))/(2*DM4th_i);
    }

    template<class T>
    inline _number<T> cos(_number<T> n) {
        return (isReal(n))?std::cos(n.real()) : (pow(e, DM4th_i*n)+pow(e, -DM4th_i*n))/2;
    }
    ...
    template<class T> inline _number<T> cot(_number<T> n) {
        return 1/tan(n);
    }
    template<class T> inline _number<T> sec(_number<T> n) {
        return 1/cos(n);
    }
    template<class T> inline _number<T> csc(_number<T> n) {
        return 1/sin(n);
    }

    template<class T> inline _number<T> asin(_number<T> n) {
        return -DM4th_i*ln(DM4th_i*n+sqrt(1-n*n));
    }
    template<class T> inline _number<T> acos(_number<T> n) {
        return pi/2-asin(n);
    }
    template<class T> inline _number<T> atan(_number<T> n) {
        return DM4th_i/2*(ln(1-DM4th_i*n)-ln(1+DM4th_i*n));
    }
    template<class T> inline _number<T> acot(_number<T> n) {
        return atan(1/n);
    }
    template<class T> inline _number<T> asec(_number<T> n) {
        return acos(1/n);
    }
    template<class T> inline _number<T> acsc(_number<T> n) {
        return asin(1/n);
    }

    template<class T> inline _number<T> sinh(_number<T> n) {
        return (pow(e, n)-pow(e, -n))/2;
    }
    template<class T> inline _number<T> cosh(_number<T> n) {
        return (pow(e, n)+pow(e, -n))/2;
    }
    template<class T> inline _number<T> tanh(_number<T> n) {
        return sinh(n)/cosh(n);
    }
    template<class T> inline _number<T> coth(_number<T> n) {
        return cosh(n)/sinh(n);
    }
    template<class T> inline _number<T> sech(_number<T> n) {
        return 1/cosh(n);
    }
    template<class T> inline _number<T> csch(_number<T> n) {
        return 1/sinh(n);
    }

    template<class T> inline _number<T> asinh(_number<T> n) {
        return ln(n+sqrt(n*n+1));
    }
    template<class T> inline _number<T> acosh(_number<T> n) {
        return ln(n+sqrt(n*n-1));
    }
    template<class T> inline _number<T> atanh(_number<T> n) {
        return 0.5*ln((1+n)/(1-n));
    }
    template<class T> inline _number<T> acoth(_number<T> n) {
        return atanh<T>(1/n);
    }
    template<class T> inline _number<T> asech(_number<T> n) {
        return acosh<T>(1/n);
    }
    template<class T> inline _number<T> acsch(_number<T> n) {
        return asinh<T>(1/n);
    }
