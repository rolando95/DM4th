...
namespace DM4th
{
    ...
    {
        if (this->rank() <= 1)
            this->_resize1DArray(axis1); // 0 || 1 dim to 1 dim
        else
        { // N dim to 1 dim
    ...
    const NDArray<T> &NDArray<T>::reshape(const int &axis1, U... args)
    {
        ...
        if (oldShapeMult != newShapeMult)
            this->_data->array.resize(newShapeMult);
    ...
    void NDArray<T>::push(const U &value, const int &pos)
    {
        ...
        if (pos == END)
            this->_data->array.push((T)value, this->data_size());
        else
            this->_data->array.push((T)value, pos);
    ...
    T NDArray<T>::pop(const int &idx)
    {
        ...
        T result;
        if (idx == END)
            result = this->_data->array.pop(this->data_size() - 1);
        else
            result = this->_data->array.pop(idx);
