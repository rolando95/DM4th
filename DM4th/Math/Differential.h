...
inline number derivative(const std::function<number(number)> &f, const number x0, const number order=1, const number tolerance=1e-2);
/*
...
inline number integral(const std::function<number(number)> &f, const number&, const number&, const number subintervals=1000);
